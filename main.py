...
class TelaPrincipal(Gtk.Box):
    ...
    def __init__(self, office_ip=""):
        ...
        self.entrada_dominio = Gtk.Entry(
        ...
        caixa_entrada.append(self.entrada_dominio)
        caixa_entrada.append(self.botao_adicionar)
        add_box.append(caixa_entrada)
        add_box.append(self.spinner_add)
        ...
        janela_rolagem = Gtk.ScrolledWindow(
            child=self.lista_rotas, vexpand=True, min_content_height=200
        )
    ...
    def on_add_route_clicked(self, widget):
        ...
        self.control.add_route_to_list(
            domain, self.handle_route_add_success, self.handle_route_add_error
        )
    ...
    def on_remove_route_request(self, route_row):
        ...
        self.vpn_web.remove_route(
            ...
            lambda err: route_row.on_remove_error(
                err
            ),  # Passa a função de erro da própria linha
        )
...
class TelaLogin(Gtk.Box):
    ...
    def __init__(self):
        ...
        self.entrada_senha = Gtk.Entry(
            placeholder_text=_("Password"),
            visibility=False,
            input_purpose=Gtk.InputPurpose.PASSWORD,
        )
        checkbox = Gtk.CheckButton(label=_("Keep Info"))
        ...
        if self.is_checked:
            self.entrada_website.set_text(data.get("server", ""))
            self.entrada_usuario.set_text(data.get("username", ""))
            self.entrada_senha.set_text(data.get("password", ""))
        ...
        self.append(self.entrada_website)
        self.append(Gtk.Label(label=_("User"), xalign=0))
        self.append(self.entrada_usuario)
        self.append(Gtk.Label(label=_("Password"), xalign=0))
        self.append(self.entrada_senha)
        self.append(checkbox)
        self.append(self.botao_entrar)
        self.append(self.spinner)

    def on_botao_entrar_clicado(self, widget):
        self.botao_entrar.set_sensitive(False)
        self.spinner.start()
        self.control.get_user_login_info(
            self.entrada_website.get_text(),
            self.entrada_usuario.get_text(),
            self.entrada_senha.get_text(),
            self.is_checked,
        )
        vpn = VpnCon(
            self.entrada_website.get_text(),
    ...
    def on_login_success(self, office_mode_ip):
        self.last_office_ip = office_mode_ip
        self.spinner.stop()
        modal = Adw.MessageDialog(
            transient_for=self.get_root(),
            title=_("Login Successful"),
            body=_("You are now connected!"),
        )
        modal.add_response("ok", _("OK"))
        modal.connect("response", self.on_dialog_response)
        modal.present()

    def on_dialog_response(self, dialog, response):
        dialog.close()
        self.emit("login-sucesso", self.last_office_ip)

    def on_login_error(self, error_message):
        self.botao_entrar.set_sensitive(True)
        self.spinner.stop()
        if "Another session" in error_message:
            self.emit("login-sucesso", "")
            return
        modal = Adw.MessageDialog(
            transient_for=self.get_root(),
        ...
        modal.add_response("ok", _("OK"))
        modal.set_response_appearance("ok", Adw.ResponseAppearance.DESTRUCTIVE)
        modal.connect("response", lambda d, r: d.close())
        modal.present()
...
class JanelaPrincipal(Gtk.ApplicationWindow):
    ...
    def mostrar_tela_principal(self, widget, office_ip):
        if self.stack.get_child_by_name("principal"):
            self.stack.remove(self.stack.get_child_by_name("principal"))
    ...
    def on_disconnect_clicked(self, widget):
        ...
        VpnDiss(
            on_success=self.on_disconnect_success, on_error=self.on_disconnect_error
        )
    ...
    def on_activate(self, app):
        ...
        def on_error(arg):
            ...
            dialog = Adw.MessageDialog(
                ...
                body=_("Please install the required dependencies."),
            )
            dialog.add_response("ok", _("OK"))
            dialog.set_response_appearance("ok", Adw.ResponseAppearance.DESTRUCTIVE)
            dialog.connect("response", lambda d, r: d.close())
